...
function restore_sqlfromtemp {
    ...
    MYSQLFILE=$DATADIR/backup/mysqldb_asterisk/asterisk.sql
    TABLES=$(grep "^LOCK TABLES" $MYSQLFILE  | grep -v 'TABLES `notifications' | grep -v 'TABLES `modules' | grep -v 'TABLES `module_xml' | grep -v 'TABLES `issabelpbx_settings' | grep -v 'TABLES `admin' | cut -d' ' -f 3 | sed 's/`//g' | xargs)
